...
class EndOfCentralDirectoryRecord:
    ...
    def __init__(
        ...
        self.disk_where_central_directory_starts = (
            disk_where_central_directory_starts
        )
        self.number_of_central_directory_records_on_this_disk = (
            number_of_central_directory_records_on_this_disk
        )
    ...
    def parse(cls, apk_file):
        ...
        disk_where_central_directory_starts = struct.unpack(
            '<H', apk_file.read(2)
        )[0]
        number_of_central_directory_records_on_this_disk = struct.unpack(
            '<H', apk_file.read(2)
        )[0]
        total_number_of_central_directory_records = struct.unpack(
            '<H', apk_file.read(2)
        )[0]
        size_of_central_directory = struct.unpack('<I', apk_file.read(4))[0]
        offset_of_start_of_central_directory = struct.unpack(
            '<I', apk_file.read(4)
        )[0]
        comment_length = struct.unpack('<H', apk_file.read(2))[0]
        comment = struct.unpack(
            f'<{comment_length}s', apk_file.read(comment_length)
...
class CentralDirectoryEntry:
    ...
    def __init__(
        ...
        self.relative_offset_of_local_file_header = (
            relative_offset_of_local_file_header
        )
...
class CentralDirectory:
    ...
    def parse(cls, apk_file, eocd: EndOfCentralDirectoryRecord = None):
        ...
        while True:
            ...
            version_needed_to_extract = struct.unpack('<H', apk_file.read(2))[
                0
            ]
            general_purpose_bit_flag = struct.unpack('<H', apk_file.read(2))[0]
            compression_method = struct.unpack('<H', apk_file.read(2))[0]
            file_last_modification_time = struct.unpack(
                '<H', apk_file.read(2)
            )[0]
            file_last_modification_date = struct.unpack(
                '<H', apk_file.read(2)
            )[0]
            crc32_of_uncompressed_data = struct.unpack('<I', apk_file.read(4))[
                0
            ]
            ...
            disk_number_where_file_starts = struct.unpack(
                '<H', apk_file.read(2)
            )[0]
            internal_file_attributes = struct.unpack('<H', apk_file.read(2))[0]
            external_file_attributes = struct.unpack('<I', apk_file.read(4))[0]
            relative_offset_of_local_file_header = struct.unpack(
                '<I', apk_file.read(4)
            )[0]
            filename = struct.unpack(
                f'<{file_name_length}s', apk_file.read(file_name_length)
            )[0].decode('utf-8', 'ignore')
            extra_field = struct.unpack(
                f'<{extra_field_length}s', apk_file.read(extra_field_length)
            )[0].decode('utf-8', 'ignore')
            file_comment = struct.unpack(
                f'<{file_comment_length}s', apk_file.read(file_comment_length)
            )[0].decode('utf-8', 'ignore')
            ...
            central_directory_entries[central_directory_entry.filename] = (
    ...
    def parse(cls, apk_file, entry_of_interest: CentralDirectoryEntry):
        ...
        if not header_signature == b'\x50\x4b\x03\x04':
            ...
            try:
                ...
                extra_field = struct.unpack(
                    f'<{extra_field_length}s',
                    apk_file.read(extra_field_length),
                )[0].decode('utf-8', 'ignore')
            except:
...
class ZipEntry:
    ...
    def parse_single(
        ...
        apk_file,
        filename,
        eocd: EndOfCentralDirectoryRecord = None,
        ...
        if not eocd or not central_directory:
            ...
            central_directory = CentralDirectory.parse(apk_file, eocd)
        local_header = {
            filename: LocalHeaderRecord.parse(
    ...
    def to_dict(self):
        ...
        return {
            ...
            "local_headers": {
                filename: entry.to_dict()
                for filename, entry in self.local_headers.items()
            },
        }
    ...
    def read(self, name):
        ...
        return extract_file_based_on_header_info(
            ...
            self.get_central_directory_entry_dict(name),
        )[0]
